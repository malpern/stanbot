...
// between the droop and vertical (level +288), and a lost face returns it to level (614,
...
void measuredPitchLevelFromDroop() {
  ...
  assert(tracker.pitchHigh() == 614 + 288);   // vertical
  ...
  for (int i = 0; i < 120; ++i) {
    ...
    const FollowCommand command = tracker.step(now);
    assert(command.pitch >= 594 && command.pitch <= 614 + 288);
  }
