...
/// The named states both faces must agree about. Deliberately a fixed list
/// rather than "every emotion": what matters is the states a person can SEE
/// the robot in, including the ones that are awkward to reach on hardware.
//
// `lids` is how far the eyes are OPEN, 1 to 0, and it is an INPUT: the state
// says what the lids are, and both sides compute the same face from it. The
// first version instead ran the sleep animation and reported the lid position
// back as a rounded percentage, which no one could reproduce exactly -- the
// two faces then disagreed by a pixel on a half-closed eye for no better
// reason than rounding. Specify the input; compare the output.
struct State { const char* name; StanbotEmotion emotion; double lids; bool speaking; };
static const State kStates[] = {
    {"normal", StanbotEmotion::Normal, 1.0, false},
    {"happy", StanbotEmotion::Happy, 1.0, false},
    {"sad", StanbotEmotion::Sad, 1.0, false},
    {"focused", StanbotEmotion::Focused, 1.0, false},
    {"sleepy", StanbotEmotion::Sleepy, 1.0, false},
    {"surprised", StanbotEmotion::Surprised, 1.0, false},
    {"squint", StanbotEmotion::Squint, 1.0, false},
    {"trouble", StanbotEmotion::Trouble, 1.0, false},
    {"half_closed", StanbotEmotion::Normal, 0.5, false},
    {"nearly_shut", StanbotEmotion::Normal, 0.1, false},
    {"closed", StanbotEmotion::Normal, 0.0, false},
    {"trouble_closed", StanbotEmotion::Trouble, 0.0, false},
    {"speaking", StanbotEmotion::Normal, 1.0, true},
};

static stanbot::FaceGeometry geometryOf(const State& state) {
    StanbotEyes eyes;
    eyes.begin(0);
    eyes.setEmotion(state.emotion);
    Canvas canvas;
    uint32_t now = 0;
    for (int frame = 0; frame < 120; ++frame) { now += 34; eyes.update(canvas, now); }
    if (state.speaking) {
        // A mouth half open, as it would be mid-word.
        eyes.mouthReceive(1, 50, 0, now);
        now += 34;
        eyes.update(canvas, now);
    }
    // lids = (1 - blink) * curtain, and with no sleep asked for the curtain is
    // fully open -- so blink sets the lid position exactly, with no animation
    // to land on the wrong frame of.
    return eyes.geometry(static_cast<float>(1.0 - state.lids));
}

static const char* kindName(stanbot::EyeKind kind) {
    switch (kind) {
        case stanbot::EyeKind::Open: return "open";
        case stanbot::EyeKind::Closed: return "closed";
        case stanbot::EyeKind::Crossed: return "crossed";
    }
    return "?";
}
...
static void printGeometry() {
    std::printf("{\n");
    std::printf("  \"_comment\": \"Generated by companion/render_face.cpp --geometry. Do not edit by hand; run companion/test_face_geometry.py to regenerate.\",\n");
    std::printf("  \"screen\": {\"width\": %d, \"height\": %d},\n",
                stanbot::FaceGeometry::kScreenWidth, stanbot::FaceGeometry::kScreenHeight);
    std::printf("  \"eyeLeftX\": %d, \"eyeRightX\": %d, \"eyeY\": %d,\n",
                stanbot::FaceGeometry::kEyeLeftX, stanbot::FaceGeometry::kEyeRightX,
                stanbot::FaceGeometry::kEyeY);
    std::printf("  \"states\": {\n");
    const int count = static_cast<int>(sizeof(kStates) / sizeof(kStates[0]));
    for (int i = 0; i < count; ++i) {
        const stanbot::FaceGeometry g = geometryOf(kStates[i]);
        std::printf("    \"%s\": {\"lids\": %.2f, \"eyeKind\": \"%s\", \"eyeWidth\": %d, \"eyeHeight\": %d, "
                    "\"shut\": %d, "
                    "\"frownVisible\": %s, \"frownY\": %d, \"frownRadius\": %d, "
                    "\"mouthVisible\": %s, \"mouthX\": %d, \"mouthY\": %d}%s\n",
                    kStates[i].name, kStates[i].lids, kindName(g.eyeKind), g.eyeWidth, g.eyeHeight,
                    g.shut,
                    g.frownVisible ? "true" : "false", g.frownY, g.frownRadius,
                    g.mouthVisible ? "true" : "false", g.mouthX, g.mouthY,
                    i + 1 < count ? "," : "");
    }
    std::printf("  }\n}\n");
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--geometry") { printGeometry(); return 0; }
