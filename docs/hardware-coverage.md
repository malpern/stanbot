...
| --- | --- | --- | --- |
| Head pan servo | Calibrate safe range and direction; verify smooth bounded motion and stop behavior. | Plan-driven bounded motion (`C,YAWSWEEP`, `C,CENTER`), continuous face following, manual steering and a wake scan, with envelope guard, stall abort and independent power cutoff | Partial — a full 180 degree sweep observed 2026-09-15; sustained following at +-96 raw observed by the owner 2026-09-17 ("left and right looking good"). Wider than +-96 in following, and the wake scan, are not yet seen. |
| Head tilt servo | Calibrate safe range and direction; verify smooth bounded motion and stop behavior. | Following and manual steering between 594 and 870 raw; level measured at 614 | Partial — 2026-09-17. Up: the head meets a hard stop at ~885, which the owner saw as vertical (stall detected, power cut). Down: 592 was reached pressing against something; the limit is the unpowered rest, 594. Level was judged by eye and does not fit the measured scale, so it is due a re-check. |
...
| Wi-Fi | Verify local connection, reconnect behavior, and operation without internet. | Camera stream, commands, OTA and a UDP mouth channel; passphrase challenge for motion, reboot and power-off; multi-network profiles | Yes — in daily use since 2026-09-16: stream, following, OTA flashing (dozens of times) and reconnect after reboot. Operation without internet: not deliberately tested. |
| Bluetooth | Verify discovery and an appropriate supported local data exchange. | No | No |
| RGB LEDs | Exercise each LED and channel at bounded brightness. | The body's 12-LED bar as one colour: blue with a face, a slow orange breath without, dark asleep, a ramp on waking | Partial — blue and the orange breath confirmed by the owner 2026-09-17. Individual LEDs are not addressed separately. After a sleep and wake: not yet confirmed (an I2C fault kept it dark that afternoon; fixed). |
| Battery and power | Compare reported battery/charging state with USB and battery operation. | Backlight and power-off through the AXP2101 from the camera task's I2C driver (`backlight.h`); sleep and wake; base link health (`SBHL`). Battery and charging state are not read. | Partial — 2026-09-17: sleep and wake cycles leave the base reachable (`tools/check_sleep_wake.py`). Power-off (`C,OFF`) has never been run. Battery state: No. |
